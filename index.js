...
client.on(Events.MessageCreate, async (msg) => {
    try {
        ...
        if (applications.has(awaitKey)) {
            ...
            // Уведомляем только игрока (ephemeral нельзя в MessageCreate, поэтому шлём в ЛС)
            await msg.author.send("✅ **Ваш отчёт отправлен на проверку!** Ожидайте решения администрации.").catch(() => null);
            return;
        }
        ...
        if (i.isButton() && i.customId.startsWith("mp_accept_")) {
            ...
            if (panelChannelId) {
                ...
                if (panelChannel) {
                    ...
                    // Удаляем уведомление через 10 секунд чтобы не засорять канал
                    if (notifMsg) setTimeout(() => notifMsg.delete().catch(() => null), 10000);
                }
            }
            return;
        }
