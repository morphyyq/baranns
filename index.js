...
Капт: \`+20\` WIN | \`+10\` LOSE`;
            ...
            if (i.commandName === "panel") {
                ...
                const PANEL_BANNER_URL = "https://media.discordapp.net/attachments/1506711652483993686/1521263285612187954/0af710cc-92c3-4114-b5e4-6b45be119369.png?ex=6a443256&is=6a42e0d6&hm=bccab2d3a8d9aaabcd4db67048d4673d172bfc945f2e52f544c2ea4182ea7689&=&format=webp&quality=lossless&width=1876&height=625";
                            ...
                            components: [
                                ...
                                { type: 14 }, // Separator
                                {
                                    ...
                                    components: [
                                        {
                                            type: 10,
                                            content: "**main**\nповышение до роли main\nАвтоматически выдаёт роль <@&1458485277495656553>.\nЦена: 500"
                                        }
                                    ],
                                    accessory: {
                                        type: 2,
                                ...
                                },
                                {
                                    ...
                                    components: [
                                        ...
                                        },
                                        {
                                            ...
                                            style: 2,
                                            label: "РП отчёт",
                                            custom_id: "interaction_rp_report",
                                            emoji: { name: "📋" }
                                        }
                                    ]
                                }
...
⚠️ У вас есть **2 минуты**, иначе заявка отменится.`,
        ...
        if (i.isButton() && i.customId === "interaction_portfolio") {
            ...
            const existingChannel = i.guild.channels.cache.find(
                c => c.name === `katanych-${i.user.username.toLowerCase().replace(/[^a-z0-9]/g, "")}` ||
            ...
            const portfolioChannel = await i.guild.channels.create({
                name: `katanych-${i.user.username.toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 20)}`,
...
` +
        ...
        if (i.isButton() && i.customId === "shop_balance") {
            ...
            await i.reply({ content: `🏆 Баланс баллов: \`${currentPoints}\``, ephemeral: true });
            return;
        }
        ...
        if (i.isButton() && i.customId === "shop_buy_warn") {
            ...
            if (currentPoints < price) {
                await i.reply({ content: `❌ Недостаточно баллов. Нужно **${price}**, у вас **${currentPoints}**.`, ephemeral: true });
                return;
            }
            ...
            await i.reply({ content: `✅ Покупка оформлена: **Снять выговор**. Списано **${price}** баллов. Остаток: **${salary.mpPoints[i.user.id]}**.\n⚠️ Обратитесь к администрации для снятия предупреждения.`, ephemeral: true });

            const logChannel = await i.guild.channels.fetch("1518544382985371698").catch(() => null);
            if (logChannel) {
                await logChannel.send({ content: `🛒 <@${i.user.id}> купил(а) **«Снять выговор»** за **${price}** баллов. Требуется ручное снятие предупреждения администрацией.` }).catch(() => null);
            }
            return;
        }
        ...
        if (i.isButton() && i.customId === "shop_buy_cash") {
            ...
            await i.reply({ content: `✅ Покупка оформлена: **100,000 игровой валюты**. Списано **${price}** баллов. Остаток: **${salary.mpPoints[i.user.id]}**.\n💵 Обратитесь к администрации для ручной выдачи.`, ephemeral: true });
        ...
        if (i.isButton() && i.customId === "shop_buy_main") {
            ...
            await i.reply({ content: `✅ Покупка оформлена: повышение до роли **main**. Списано **${price}** баллов. Остаток: **${salary.mpPoints[i.user.id]}**.\n📈 Роль <@&${MAIN_ROLE_ID}> выдана автоматически.`, ephemeral: true });
