    ...
    try {
        ...
        if (i.isChatInputCommand()) {
            ...
            if (i.commandName === "panel") {
                ...
                // Подгружаем растянутое изображение Darkness.jpg из корня проекта
                const imagePath = path.join(__dirname, "Darkness.jpg");
                const hasImage = fs.existsSync(imagePath);
                const file = hasImage ? new AttachmentBuilder(imagePath, { name: "Darkness.jpg" }) : null;

                const embed = new EmbedBuilder()
                    .setTitle("✝️ Принимаем заявки на две роли ACADEMY / CAPTURE ✝️")
                    .setDescription(
`**На роль CAPTURE** требуются откаты с MCL/CAPT **И** GUNGAME.
Если претендуете на роль **ACADEMY** — откаты с GUNGAME (арены).

**Срок рассмотрения заявок: от 3 до 5 рабочих дней.** Решение направляется ботом в личные сообщения + открытие ветки, где вас скорее всего попросят скинуть скриншот персонажей и вызовут на **ОБЗВОН**. Отсутствие ответа в указанный срок означает отказ в заявке.

> ⚠️ Заявки в семью принимаются только на сервер **Orlando. (18)**
> **ВСЕ ЗАЯВКИ МИНИМУМ ОТ 15 ЛЕТ!**

Внимательно прочитайте шаблон заявки при её подаче — там тоже есть информация.
В заявке на роль CAPTURE требуются полные откаты с GG и МП (MCL, ВЗЗ, КАПТЫ).
...
> ⚠️ После подачи заявки **следите за ЛС** или за запросами на общение в Discord. На обзвон рекрут позовёт вас **только в Discord DARKNESS GANG** — ни в ЛС, ни в другой сервер.
...
• **Внимательно ознакомьтесь с условиями выше и затем выберите тип заявки.**`
                    )
                    .setColor("#2b2d31");

                if (file) {
                    embed.setImage("attachment://Darkness.jpg");
                }

                const menu = new ActionRowBuilder().addComponents(
                    new StringSelectMenuBuilder()
                        .setCustomId("apply_menu")
                        .setPlaceholder("ACADEMY или CAPTURE")
                        .addOptions(
