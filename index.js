...
> ⏱️ **Срок рассмотрения заявки:** от **1 до 5 дней**.
> ⚠️ **Важно:** если у вас нет подходящих откатов — заявка будет **отклонена**.

━━━━━━━━━━━━━━━━━━━━━━━━━━━`
                    )
                    .addFields(
                        {
                            name: "📋 Требования к откатам",
                            value: [
                        ...
                        {
                            name: "🎯 Направления набора",
                            value: [
                                "🔹 **Academy** — базовое вступление в семью.",
                                "🔸 **Capture** — для тех, кто участвует в захватах.",
                            ].join("\n"),
                            inline: false
                        },
                        {
                            name: "🔒 Статус набора",
                            value: "> Подать заявку можно **только при открытом наборе**. Если кнопка недоступна — набор **закрыт**.",
                            inline: false
                        }
                    )
                    .setFooter({ text: "DARKNESS FAMQ • Система заявок" })
                    .setTimestamp();

                const menu = new ActionRowBuilder().addComponents(
                    new StringSelectMenuBuilder()
                        .setCustomId("apply_menu")
                        .setPlaceholder("📂 Нажмите, чтобы выбрать тип заявки")
                        .addOptions(
