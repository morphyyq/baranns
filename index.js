...
client.on(Events.MessageCreate, async (msg) => {
    try {
        ...
        if (applications.has(awaitKey)) {
            ...
            const safeId = `${msg.author.id}_${mpData.mpType.replace(/ /g,"")}_${mpData.result}_${mpData.points}`;
            ...
            await msg.reply({ content: "✅ Ваш отчёт отправлен на проверку! Ожидайте решения администрации." });
            return;
        }
...
**Шаг 2:** Выберите результат:`,
        ...
        if (i.isStringSelectMenu() && i.customId.startsWith("mp_select_result_")) {
            ...
            applications.set(`mp_await_${i.user.id}`, { mpType, result, points });

            await i.update({
                content: `✅ **МП:** ${mpType} | **Результат:** ${result === "win" ? "Win ✅" : "Lose ❌"} | **Баллы:** +${points}

📸 **Шаг 3:** Теперь отправьте скриншот подтверждения в **этот канал** (просто прикрепите картинку к сообщению).`,
                components: []
            });
            return;
        }
        ...
        if (i.isButton() && i.customId.startsWith("mp_accept_")) {
            ...
            // mp_accept_USERID_MP_RESULT_POINTS
            ...
            if (targetUser) {
                await targetUser.send(`✅ **Ваш МП отчёт принят!**\n🎮 **МП:** ${mpType} | ${result === "win" ? "Win" : "Lose"}\n🏆 **Начислено баллов:** +${points}\n📊 **Всего баллов:** ${salary.mpPoints[userId]}`).catch(() => null);
            }
            return;
        }
