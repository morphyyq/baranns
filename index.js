            ...
            if (i.commandName === "panel") {
                ...
                collector.on("collect", async (m) => {
                   ...
                   const embed = new EmbedBuilder()
                        ...
                        .setImage(imageUrl)
                        .setDescription(
`## <:hello:1506315095335243849> Путь в семью начинается здесь!

-# <:df:1506315338625585263> Заявки в семью принимаются только на сервере <:memphisLogo:1515984688643047494> **Denver**. Уведомление о приглашении на обзвон отправляется в ЛС и в канал.
<:df:1506315338625585263>**Внимательно прочитайте все пункты** при подаче заявки. **Если не ответили на все пункты** — заявка будет **отклонена**.
...
<:df:1506315338625585263> Откаты с GG — не более 1 недели назад (не менее 6 минут).
<:df:1506315338625585263> Откаты с МП (ВЗЗ, MCL, Capt) — не более 60 дней назад. — **__при наличии!__**
<:df:1506315338625585263> Откаты должны быть не в виде мувика/нарезки.
<:df:1506315338625585263> Откаты должны быть с сайги и со спешика (минимум 2 отката).
<:df:1506315338625585263> Подать заявку можно только при открытом наборе. Если нет доступа к подаче — набор закрыт.
