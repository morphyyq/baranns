...
async function updateSalaryEmbed(guild) {
    try {
        ...
        for (const [recruiterId, bal] of Object.entries(salary.balances)) {
            if (bal  0) {
                listString += `• @${recruiterId} — $${bal.toLocaleString()}n`;
            ...
            for (const roleData of config.MONITOR_ROLES) {
                ...
                if (matchedMembers.length === 0) {
                    ...
                    matchedMembers.forEach(member = {
                        ...
                        listString += `@${member.id} — ${statusEmoji}n`;
                    });
                }
...
async function updateAFKEmbed(guild) {
    try {
        ...
        let description = 📋 Список активных участников в АФК режимеnn;
        ...
        if (afkEntries.length === 0) {
            ...
            afkEntries.forEach(([userId, timestamp]) = {
                ...
                description += `• @${userId} — Встал в АФК t${timeUnix}R (t${timeUnix}t)n`;
            });
        }
...
client.once(Events.ClientReady, async () = {
    ...
    const commands = [
        ...
        new SlashCommandBuilder().setName(main_panel).setDescription(Отправить панель заявок в Main состав),
        ...
        if (msg.channel.name.startsWith(closed-)) {
            ...
            if (auditChannel) {
                ...
                const auditMsg = await auditChannel.send({ 
                    content `📋 Отчёт по принятой заявкеn👤 Рекрутер @${msg.author.id}n👤 Принятый кандидат ${candidateText}n📂 Тикет `${msg.channel.name}``,
                    files [file] 
                });
        ...
        if (i.isChatInputCommand()) {
            ...
            if (i.commandName === all) {
                ...
                try {
                    ...
                    for (const [id, member] of targetMembers) {
                        try {
                            await member.send(`🔔 Оповещение от @${i.user.id}nn## ${textMsg} ##`);
                            successCount++;
                        } catch (e) {}
                    }
            ...
            if (i.commandName === rank) {
                ...
                const rankEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`👤 Пользователь @${targetUser.id}nℹ️ Текущий ранг `${currentRankName}`n✅ Всего одобренных отчетов `${totalReports}``)
                    .setColor(#2b2d31)
                    .setTimestamp();
            ...
            if (i.commandName === info) {
                ...
                const infoEmbed = new EmbedBuilder()
                    ...
                    .setThumbnail(targetUser.displayAvatarURL({ dynamic true }))
                    .setDescription(`👤 Пользователь @${targetUser.id}n🆔 Discord ID `${targetUser.id}`n📝 Кто принял в тикете ${acceptedByText}n⏳ Времени на сервере `${daysOnServer} дней` (c ${targetMember.joinedAt.toLocaleDateString(ru-RU)})`)
                    .setColor(#2b2d31);
            ...
            if (i.commandName === panel) {
                ...
                const embed = new EmbedBuilder()
                    .setColor(#2b2d31)
                     .setImage(attachmentbanner.png)  Эту строку мы убрали
                    .setDescription(
`## hello1516906998715912334 Путь в семью начинается здесь!

-# df1516907994552602634 Заявки в семью принимаются только на сервере Denver. 
df1516907994552602634 Внимательно прочитайте все пункты при подаче заявки. Если не ответили на все пункты — заявка будет отклонена.
...
df1516907994552602634 Откаты с GG — не более 1 недели назад (не менее 6 минут).
df1516907994552602634 Откаты с МП (ВЗЗ, MCL, Capt) — не более 60 дней назад. — __при наличии!__
df1516907994552602634 Откаты должны быть не в виде мувиканарезки.
df1516907994552602634 Откаты должны быть с сайги и со спешика (минимум 2 отката).
df1516907994552602634 Подать заявку можно только при открытом наборе. Если нет доступа к подаче — набор закрыт.
・Выберите пункт в выпадающем меню`
                ...
                const menu = new ActionRowBuilder().addComponents(
                    new StringSelectMenuBuilder()
                        ...
                        .addOptions(
                            { label Academy, description Ник, статик, имявозраст, онлайн, семья, value academy },
                            { label Capture, description Ник, статик, имявозраст, онлайн, семья, откаты, value capture }
                        )
                );
            ...
            // =====================================================
            // ПАНЕЛЬ ЗАЯВОК В MAIN СОСТАВ
            // =====================================================
            if (i.commandName === main_panel) {
                if (!config || !config.CHANNELS || !config.CHANNELS.MAIN) {
                    await i.reply({ content: '❌ Канал Main не настроен в конфиге.', ephemeral: true });
                    return;
                }
                const channel = await client.channels.fetch(config.CHANNELS.MAIN).catch(() => null);
                if (!channel) {
                    await i.reply({ content: '❌ Не удалось найти канал Main.', ephemeral: true });
                    return;
                }

                const embed = new EmbedBuilder()
                    .setColor(0x7B3FF2)
                    .setTitle('🌑 Заявка в Main состав — Darkness')
                    .setDescription(
`**Main состав** — основа нашей семьи. Здесь играют люди, готовые участвовать во всём контенте: капты, MCL, турниры и прочее.

━━━━━━━━━━━━━━

**📋 Требования для подачи:**
• Откаты с GG от 5 минут **или**
...
• Рассмотрение занимает **2–4 дня**
• Подгонять администрацию запрещено
• Отклонённая заявка — решение окончательное

━━━━━━━━━━━━━━
-# Вступая в Main состав, вы становитесь частью основного комьюнити Darkness.
・Выберите пункт в выпадающем меню ниже`
                    );

                const menu = new ActionRowBuilder().addComponents(
                    new StringSelectMenuBuilder()
                        .setCustomId('apply_main_menu')
                        .setPlaceholder('Нажмите, чтобы подать заявку в Main состав')
                        .addOptions(
                            { label: 'Main состав', description: 'Ник, статик, имя/возраст, онлайн, откаты', value: 'main' }
                        )
                );

                await channel.send({ embeds: [embed], components: [menu] });
                await i.reply({ content: '✅ Панель заявок в Main состав успешно отправлена!', ephemeral: true });
                return;
            }
                const embed = new EmbedBuilder()
...
⚠️ Повышение не выдаётся автоматически без ручного одобрения старшего состава в планшете. Нажмите кнопку ниже, чтобы прикрепить доказательства.`)
            ...
            if (i.commandName === afk_panel) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(Используйте интерактивные переключатели ниже для изменения своей активности на сервере.nПри нахождении в АФК-режиме, вам не будут рассылаться спам-уведомления о сборах групп.)
                    .setColor(#2b2d31);
            ...
            if (i.commandName === group_panel) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(
                        Используйте кнопки ниже для запуска ручного управления сборами состава.nn +
                        Функционалn +
                        • Выбор типа мероприятияn +
                        • Ручная панель с кнопками отправки в канал и ЛСnn +
        ...
        if (i.isButton() && i.customId.startsWith(view_archive_app_)) {
            ...
            const appEmbed = new EmbedBuilder()
                ...
                .setDescription(`Статик и Никнейм ${arch.fields.q1}nnИмя и Возраст ${arch.fields.q2}nnОпыт ${arch.fields.q3}nnПочему именно мы ${arch.fields.q4}${arch.fields.q5  `nnОткаты ${arch.fields.q5}`  }`)
                .setColor(#1f8b4c);
        ...
        if (i.isModalSubmit() && i.customId === modal_report_submit) {
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`👤 Отправитель @${i.user.id}n🆔 Статик `${staticIdStr}`n🔗 Доказательства ${proofLink}`)
                .setColor(#2b2d31)
                .setTimestamp();
                ...
                if (triggerPromo) {
                    ...
                    if (notifyChannel) {
                        const promoEmbed = new EmbedBuilder()
                            ...
                            .setDescription(`👤 Игрок @${targetId} успешно выполнил требования по количеству отчетов (${currentCount} шт.).nПожалуйста, подтвердите его повышение в планшете с ${fromRankName} до ${toRankName}.`)
                            .setColor(Purple);
        ...
        if (i.isModalSubmit() && i.customId.startsWith(group_modal_code_)) {
            ...
            const controlEmbed = new EmbedBuilder()
                ...
                .setDescription(`Фракция ${faction.toUpperCase()}nМероприятие ${activity}nКод группы `${code}`nnИспользуйте кнопки ниже для рассылки. Кнопку в канал можно нажимать много раз для спама.`)
                .setColor(Yellow);
        ...
        if (i.isButton() && i.customId.startsWith(sbor_)) {
            ...
            const messageContent = `${pingString}nn## Сбор на ${activity}, всем быть, кого не будет = 2 варна. Группа ${code} ##`;
            ...
            if (action === channel) {
                ...
                try {
                    ...
                    for (const [id, member] of targetMembers) {
                        try {
                            await member.send(`🔔 Внимание!n${messageContent}`);
                            successCount++;
                        } catch (e) {}
                    }
            ...
            if (logChannel) {
                ...
                if (originalEmbed) {
                    ...
                    const rejectEmbed = new EmbedBuilder()
                        ...
                        .setDescription(`👤 Кандидат @${targetId}n🔒 Отклонил @${i.user.id}n📝 Причина ${reason}`)
                        .setColor(Red)
                        .setTimestamp();
                    await logChannel.send({ embeds [rejectEmbed] }).catch(() = null);
                }
        ...
        // =====================================================
        // ВЫПАДАЮЩЕЕ МЕНЮ — ЗАЯВКА В MAIN СОСТАВ
        // =====================================================
        if (i.isStringSelectMenu() && i.customId === 'apply_main_menu') {
            const modal = new ModalBuilder()
                .setCustomId('apply_modal_main')
                .setTitle('Заявка в Main состав');

            const fields = [
                { id: 'mq1', label: 'СТАТИК # И НИК', placeholder: '21074 | Hugo Darkness', style: TextInputStyle.Short },
                { id: 'mq2', label: 'ИМЯ И ВОЗРАСТ', placeholder: 'Женя | 20', style: TextInputStyle.Short },
                { id: 'mq3', label: 'ОНЛАЙН В ДЕНЬ (ЧАСОВ)', placeholder: 'Примерно 4–5 часов', style: TextInputStyle.Short },
                { id: 'mq4', label: 'ОПЫТ В СЕМЬЯХ (ГДЕ СОСТОЯЛИ)', placeholder: 'Да, был в... / Нет', style: TextInputStyle.Paragraph },
                { id: 'mq5', label: 'ОТКАТЫ (ССЫЛКА ИЛИ ОПИСАНИЕ)', placeholder: 'Ссылка на откат с ГГ от 5 мин / МП / капта', style: TextInputStyle.Paragraph }
            ];
        ...
        // =====================================================
        // ОБРАБОТКА МОДАЛА ЗАЯВКИ В MAIN СОСТАВ
        // =====================================================
        if (i.isModalSubmit() && i.customId === 'apply_modal_main') {
            if (modalLocks.has(i.user.id)) return;
            modalLocks.add(i.user.id);
            setTimeout(() => modalLocks.delete(i.user.id), 5000);

            const expectedChannelName = `main-${i.user.username}`.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9\-_]/g, '');
            ...
            const data = {
                type: 'main',
                mq1: i.fields.getTextInputValue('mq1'),
                mq2: i.fields.getTextInputValue('mq2'),
                mq3: i.fields.getTextInputValue('mq3'),
                mq4: i.fields.getTextInputValue('mq4'),
                mq5: i.fields.getTextInputValue('mq5'),
                userId: i.user.id
            };
...
<@${i.user.id}>`;
            ...
            await ticketChannel.send({ content: topContent, embeds: [embed], components: [row] });
            await i.reply({ content: `✅ Заявка в Main состав создана! <#${ticketChannel.id}>`, ephemeral: true });
            return;
        }
        ...
        if (i.isModalSubmit() && i.customId.startsWith(apply_modal_)) {
            ...
            const rolesPing = config.ALLOWED_ROLES  config.ALLOWED_ROLES.map(r = `@&${r}`).join( )  ;
            const topContent = `${rolesPing} @&1468704257606684712nПредыдущие заявкиnЗаявок не найдено.`;
...
${data.q4}`;
            ...
            if (type !== academy) {
                embedDescription += `nnПредоставьте свои откатыn${data.q5}`;
            }

            embedDescription += `nnПользовательn@${i.user.id}`;
        ...
        if (i.isChannelSelectMenu() && i.customId.startsWith(call_voice_)) {
            ...
            const voiceUrl = `httpsdiscord.comchannels${i.guild.id}${voiceChannelId}`;

            await i.channel.send(`📞 @${targetId}, вы вызваны на обзвон администратором @${i.user.id}!nПожалуйста, перейдите в голосовой канал [Войти в голосовой канал](${voiceUrl}) (#${voiceChannelId}).`);

            const targetMember = await i.guild.members.fetch(targetId).catch(() = null);
            if (targetMember) {
                await targetMember.send({
                    content `🔔 Привет! Твоя заявка в семью Darkness на сервере ${i.guild.name} была проверена.nnТебя вызвали на обзвон! Пожалуйста, подключись к голосовой канале по прямой ссылкеn${voiceUrl}`
                }).catch(() = {
            ...
            if (parts[0] === app) {
                ...
                if (action === accept) {
                    ...
                    await i.channel.send({
                        content `🎉 @${targetId} успешно принят!nn💼 @${i.user.id}, кандидат убран из тикета. Пожалуйста, отправьте сюда скриншот с планшета, чтобы зафиксировать отчет в аудите.`
                    });
                    return;
                }
