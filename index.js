        ...
        if (i.isChatInputCommand()) {
            ...
            if (i.commandName === "panel") {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(
`**На роль CAPTURE** требуются откаты с МП/капта **И** GUNGAME.
Если претендуете на роль **ACADEMY** — откаты стрельбы не требуются.

**Срок рассмотрения заявок:** от 1 до 4 рабочих дней. Решение направляется ботом в личные сообщения + открытие ветки, где вас скорее всего попросят скинуть скриншот персонажа и вызовут на **ОБЗВОН**. Отсутствие ответа в указанный срок означает отказ в заявке.

> ⚠️ **Заявки в семью Darkness принимаются минимум от 15 лет!**
> **ВСЕ ЗАЯВКИ СТРОГО ПО ШАБЛОНУ!**
...
> ⚠️ После подачи заявки **следите за ЛС** или за запросами на open-общение в Discord. На обзвон рекрут позовёт вас **только в Discord DARKNESS GANG** — ни в ЛС, ни в другой сервер.
...
Если вам отказали — накладывается **КД 2 дня** для повторной заявки.
